...
### Added
- Claude panes stand out in the tree: any pane running Claude Code has its title shown in yellow to distinguish it from a plain white shell. Detection is via a `ps` check on the pane's TTY, so it works for the npm/Node install too (which shows a `node` job rather than `claude`).
