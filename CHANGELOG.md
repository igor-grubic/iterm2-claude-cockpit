...
### Added
- Auto-discovered link chips on group headers. The daemon reads a status file (default `.cockpit.json`) from a group's panes' working directories and turns every URL-valued property into a chip labeled with the property name — nothing about PRs or Jira is hardcoded. A chip with a single URL opens it in your browser (`POST /api/open-url`); a property holding a list of URLs opens a popup listing them all. Non-URL properties are ignored, and groups with no status file show no chips. The status filename is configurable via `~/.config/iterm2-claude-cockpit/links.json` (`{ "file": "..." }`).
...
- Group headers now have a noticeably stronger background fill than their panes, so it's clear at a glance which row is a tab header and which are the panes underneath it — no change in row height.
- The active (focused) pane is now much easier to spot: a bolder fill and a stronger ring/accent bar than the previous subtle tint, using the group's own color (or a blue accent for uncolored groups) — no change in row height.
...
### Fixed
- Clicking a pane to focus it felt laggy: the highlight only ever moved on the next poll tick (up to ~500ms) or the daemon's periodic refresh, after the `POST /api/focus` round-trip through iTerm2 had already completed. Clicking a pane now highlights it immediately (optimistically), reconciling with the server's confirmed state on the very next poll instead of waiting on the regular interval.
